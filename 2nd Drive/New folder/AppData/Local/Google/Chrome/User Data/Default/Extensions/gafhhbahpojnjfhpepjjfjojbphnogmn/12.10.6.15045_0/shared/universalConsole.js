    ...
    var	sysConsole = window.console,
        ...
        formatter = (function(){
            var	formatValue = function(value){
                    ...
                    return type === 'object' ? formatObject(value) : String(value);
                },
                formatArray = function(array){
                    var out = [];
                    for (var i = 0; i < array.length; ++i){
                        out.push(String(array[i]));
                    }
                    return '[' + out.join(', ') + ']';
                },
                    ...
                    if (isArray(obj)){
                        ...
                        for (var p in obj){
                            if (obj.hasOwnProperty(p)){
                                var value = obj[p];
                                out += sep + p + ':' + (typeof value === 'function' ? '[function]' : value);
                                sep = ', ';
                            }
                        }
                        out = '{' + out + '}';
                    ...
                    if (args.length > 0 && typeof args[0] === 'string' && args[0].indexOf('%') != -1){
                        var re = /%[odisf%]/g,
                            substitutor = function(match /*, offset, string*/){
                                var value = match === '%' ? '%' : formatValue(args[argIndex++]);
        ...
        chromeMessengerConsole = (function(){
            var log = function(type, args){
                    try{
                        ...
                        chrome.extension.sendMessage(request, stub);
                    }catch (e){}
                },
                that = {
