...
function getSuggestions(el, URI){
	...
	requestSuggestions.onreadystatechange = function(){
		
		var state = requestSuggestions.readyState,
			status = requestSuggestions.status; 
		
		//console.log()
		if( state == 4 && status  == 200){
			displayResults(requestSuggestions.responseText);
		}
		else{
	...
	requestSuggestions.open('GET', constructedURI, true);
	requestSuggestions.send();
}
