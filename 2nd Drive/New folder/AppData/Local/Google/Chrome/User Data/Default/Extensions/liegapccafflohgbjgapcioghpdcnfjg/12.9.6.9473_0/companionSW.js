...
var CompanionSW = (function(){
    var defaultCallbacks = {},
        ...
        getExecutablePackage = function getExecutablePackage(buttonId){
            var exePackage = buttonId ? exePackageManager.getExePackageByButtonId(buttonId) : undefined;
            return exePackage;
        },

        getExecutablePackageState = function getExecutablePackageState(buttonId){
            var exePackage = getExecutablePackage(buttonId),
                state = exePackageStates[exePackage] || {};
            exePackageStates[exePackage] = state;
            return state;
        },
        ...
        downloadMissingSoftware = function downloadMissingSoftware(callbacks, buttonId){
            var isWindowsOS = navigator.platform.toUpperCase().indexOf("WIN") > -1,
                ...
                sourceId = !!buttonId ? "WidgetClick" : "Install",
                ...
                detectPackageCallback = function detectPackageCallback(state, params){
                    ...
                    case 'Missing':
                        ...
                        getEventCallback('Downloading', callbacks)(params);
                        startDownload(buttonId, exePackage, startDownloadCallback);
                        break;
                    }
                },
                startDownloadCallback = function startDownloadCallback(state, params){
                    switch (state){
                    case 'Detected':
                        sendUL(sourceId, true, exePackage.name, true);
                        getEventCallback('Installed', callbacks)();
                        break;
                    case 'Error':
                        getEventCallback('Error', callbacks)(params);
                        break;
                    case 'Missing':
                        sendUL(sourceId, true, exePackage.name, true);
                        getEventCallback('Missing', callbacks)();
                        break;
                    }
                };

            if (executablePackageState.isDownloading){
                console.log("cSW: Bypass downloadMissingSoftware - already downloading");
                getEventCallback('Error', callbacks)('Bypassed - alreadyDownloading');
                return;
            }
        ...
        detectPackage = function detectPackage(buttonId, exePackage, callback){
            ...
            detectExe({
                ...
                function resolveCallback(fileExists) {
                    ...
                    callback(fileExists ? 'Detected' : 'Missing');
                },
