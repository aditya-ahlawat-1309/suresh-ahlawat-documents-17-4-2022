	...
	initialize: function initialize(config, imageWidths, isNewInstall) {
        ...
        if (chrome.browserAction && chrome.browserAction.onClicked){
            ...
            chrome.browserAction.onClicked.addListener(
                function (tab) {
                    if (self.isSupportedScheme(tab.url) && !globalBlacklistService.isUrlBlacklisted(tab.url)) {
...
(function(){
    var toolbarDataProperties = ['toolbarId', 'partnerId', 'partnerSubId', 'installDate',
        ...
        isValidValue = function(value){
            ...
            return type != 'undefined' && value !== 'undefined' && value !== 'null';
        },
    ...
    if (!Global.isToolbarIdSet()) {
        ...
        ToolbarCookieParser.getToolbarInfo(
            ...
            function getToolbarData(toolbarData) {
                ...
                Toolbar.setupExternalStyling(function installCallback(){
                    Toolbar.initialize(config, imageWidths, true);

                    Global.postMessage('toolbar:installed');
                });
            }
        );
    } else {
        retrieveAll();
        Toolbar.setupExternalStyling(function startupCallback(){
            Toolbar.initialize(config, imageWidths, false);
        });
    }
})();
